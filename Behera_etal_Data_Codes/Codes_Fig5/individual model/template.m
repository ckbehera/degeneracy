...
[wi1,wi2,w5e1,w5e2,w5i1,w5i2,Jd5e,J55e,Jd5i,J55i,Jdi,J5di,J5ii,J5ee,Jii,w5edi]=connectdv2old();
...
for k=1:nsamples-1
    ...
    %    % Task protocols (Type II 5-HT neurons)
        ...
        % Unexpected aversive stimulus schedule on DRN's GABAergic neural pop (at US)
        I_app5i(k) =  (ave==1)*(1000)*(t(k)>5700 & t(k)<5900)*((((t(k)-5700))/tau_ext)*exp((-((t(k)-5700))/tau_ext)));
        %I_app5i = 0; % No external stimulus to DRN's GABAergic neurons
    ...
    I5i(k) = - w5i1*Id5(k) + w5i2*I5d(k) - J5ii*F5i(k) + 0*F5e(k) + I_app5i(k) + 250 ;    % Overall (weighted) input to local GABAergic population in DRN
    Ii(k) = wi1*Id5(k) + wi2*I5d(k) - Jii*Fi(k) + I_appi(k) ;                             % Overall (weighted) input to local GABAergic population in VTA
...
% Plots
% figure;
% subplot(3,3,1); plot([0:dt:dt*(length(r)-1)],r,'k'); axis tight; ylabel('Firing rate (Hz)'); ylim([0,max(r)+1]); hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.'); box off;
% title('DA firing rate');
% subplot(3,3,4); plot([0:dt:dt*(length(d)-1)],d,'k'); axis tight; ylabel('[DA] (\muM)'); ylim([0,max(d)+0.01]); hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.');box off;
% title('DA conc.')
% subplot(3,3,7); plot([0:dt:dt*(length(I_ad)-1)],I_ad,'k'); axis tight; ylabel('I_{DA auto} (a.u.)'); ylim([0,max(I_ad)+1]);  hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.');box off;
% title('DA autoinh.')
% xlabel('Time (ms)');
% subplot(3,3,2); plot([0:dt:dt*(length(r5)-1)],r5,'k'); axis tight; ylabel('Firing rate (Hz)'); ylim([0,max(r5)+1]); hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.');box off;
% title({'iapp5#0,iapp#0,J5ii=100,J5ee=0.1,Jii=100';'5-HT firing rate'});
% subplot(3,3,5); plot([0:dt:dt*(length(s)-1)],s,'k'); axis tight; ylabel('[5-HT] (\muM)'); ylim([0,max(s)+0.01]); hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.');box off;
% title('5-HT conc.');
% subplot(3,3,8); plot([0:dt:dt*(length(I_ad5)-1)],I_ad5,'k'); axis tight; ylabel('I_{5-HT auto} (a.u.)'); ylim([0,max(I_ad5)+1]);hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.');box off;
% title('5-HT autoinh.');
% xlabel('Time (ms)');
% figure;
% subplot(3,3,3); plot([0:dt:dt*(length(r))],F5i,'k'); axis tight; ylabel('Firing rate (Hz)'); ylim([0,max(F5i)+1]); hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.');box off;
% title('DRN GABA');
% subplot(3,3,6); plot([0:dt:dt*(length(r))],F5e,'k'); axis tight; ylabel('Firing rate (Hz)'); ylim([0,max(F5e)+1]); hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.');box off;
% title('DRN Glu');
% subplot(3,3,9); plot([0:dt:dt*(length(r))],Fi,'k'); axis tight; ylabel('Firing rate (Hz)'); ylim([0,max(Fi)+1]); hold on;
% plot([4500/dt 4500/dt],[0 50],'g-.'); hold on; plot([5500/dt 5500/dt],[0 50],'r-.');box off;
% title('VTA GABA');
...
xlabel('Time (ms)');
% savefig(sprintf('Expt activity prof of sero type %d in reward %d', serotype, rew));
% 
% if (rew==1 && expe==1)
...
%savefig('fn1');
%save En1.mat wi1 wi2 w5e1 w5e2 w5i1 w5i2 Jd5e J55e Jd5i J55i Jdi J5di J5ii J5ee Jii;
